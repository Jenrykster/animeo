...
async function handleWatchedEpisode(animeName, currentEpisode, token) {
  const anilistEntry = await getAnilistEntry(animeName, token);
  if (anilistEntry) {
    await updateAnilist(anilistEntry.id, currentEpisode, token);
  }
}
...
async function updateAnilist(id, currentEpisode, token) {
  const anilistApi = new Anilist(token);
